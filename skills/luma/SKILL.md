...
- For a one-off artifact or result, use the registered `luma` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi luma --help
runapi luma <operation> --help
curl --fail --location https://runapi.ai/docs/api/luma/<operation>.md --output contract.md
```
...
```shell
runapi luma <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service luma --action <operation> > result.json
```
